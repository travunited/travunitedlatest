  ...
  return (
    <AdminLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-neutral-900">Customer Report</h1>
          <p className="text-neutral-600 mt-1">Per customer lifetime value and activity</p>
        </div>
          ...
          {summary && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
              <div className="bg-white rounded-2xl shadow-medium p-6 border border-neutral-200">
                ...
                <p className="text-3xl font-bold text-neutral-900">{summary.totalCustomers}</p>
              </div>
              <div className="bg-white rounded-2xl shadow-medium p-6 border border-neutral-200">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-medium text-neutral-600">Total Lifetime Revenue</h3>
                  <DollarSign size={20} className="text-green-600" />
                </div>
                <p className="text-3xl font-bold text-green-700">₹{summary.totalLifetimeRevenue.toLocaleString()}</p>
              </div>
            </div>
          )}
              ...
              <table className="min-w-full divide-y divide-neutral-200">
                <thead className="bg-neutral-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Visa Apps</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Tour Bookings</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Lifetime Revenue</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Signup Date</th>
                ...
                <tbody className="bg-white divide-y divide-neutral-200">
                  {customers
                    ...
                    <tr key={customer.id} className="hover:bg-neutral-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-green-700">₹{customer.totalLifetimeRevenue.toLocaleString()}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral-900" suppressHydrationWarning>{formatDate(customer.createdAt)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral-900" suppressHydrationWarning>{formatDate(customer.lastActivity)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
